...
HTNode *CreatHFM(FILE * fp, short *n, WeightType * FileLength)
{
	...
	for (*FileLength = 0; !feof(fp); ++(*FileLength)) {//feof()函数的作用是判断文件指针指向文件结束 
		ch = fgetc(fp);	//fread(&ch,1,1,fp);    //fgetc() 读取成功时返回读取到的字符，读取到文件末尾或读取失败时返回EOF。
 
		ht[ch].weight++;	//printf("ht[%c].weight= %ld\n",0,ht[0].weight);
	}
	--(*FileLength);	//去掉文件结束后的长度
	*n = SortTree(ht);//sort可以快速给数组排序 
	m = *n * 2 - 1;		//free(&ht[m+1]);
	//printf("叶子个数= %d\n",*n);//getchar();
	if (1 == *n) {
		ht[0].parent = 1;
	...
	{			//在ht[0]~ht[i-1]的范围内选择两个parent为0且weight最小的结点，其序号分别赋值给s1、s2返回
		...
		SelectMinTree(ht, i - 1, &s2);//新增i-1个数据 
		ht[s2].parent = i;
		ht[i].RChild = s2;
...
void Compress()
{
	...
	printf("输入压缩文件名(例如: E:/1.txt):");
	scanf("%s", desFile);
	//getchar();//fflush(stdin);//清空输入缓冲区域

	printf("输入压缩后文件名(例如: E:/2.txt):");
	scanf("%s", rename);
	if ('\0' == rename[0]) {
		printf("输入不能为空，请重新输入:");
		scanf("%s", rename);
...
//printf("MaxLength = %d;minLen= %d,Leaf= %d\n",MaxLength,MinLength,NLeaf);getchar();
	...
	for (j = i = 0; i < Ncom; i++) {
		...
		while (Q->length > MaxLength) {
			...
			for (ch = 0; ch < num[j]; ch++) {//ch译码j是代码个数 
				Out_Queue(Q, &x);
				*p++ = x;
			}
			*p = '\0';	//printf("%c编 码%s\n",str[j],hc[j]);
